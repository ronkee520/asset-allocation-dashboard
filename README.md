...
1. 全球资产配置总览与八类资产可解释因子评分
2. 八个资产配置研究页：因子贡献、观测值、权重、来源、置信度与价格子模型验证
3. 普林格六阶段经济周期时钟：多代理组合趋势、阶段拟合、第二候选、宏观验证与来源审计
4. 20日/60日跨资产相关性与轮动矩阵：共同交易日对齐、Pearson公式、多窗口样本与代理来源
5. 全球市场行情与趋势比较
...
评分用于判断未来 1 至 3 个月的风险调整后战术配置吸引力。20日趋势、60日趋势、宏观适配、资金流或仓位、相对全球股票强弱和波动率约束分别转换为历史或同类分位，再按公开权重合成：

`展示分 = 60 + 0.30 × 综合分位`

全球股票、债券、商品、黄金和美元属于一级资产；AI、港股和A股属于权益内部方向，避免重复计算权益风险。市场风险偏好使用权益、信用、波动率、周期、美元压力和资金广度独立计算，不是八项资产分数的平均值。

用户也可以主动填写八项持仓比例，页面在浏览器中使用代表性ETF历史协方差估算组合年化波动、最大回撤、集中度和风险贡献。该输入不会上传，也不会影响其他访问者。

## 周期与相关性模型

普林格时钟分别以债券、股票和商品的多只代表性ETF构建组合趋势，采用 `35% × 20日收益 + 65% × 60日收益`，再与六阶段方向模板计算拟合度。阶段页同时公开第二候选、代理内部一致性、FRED宏观交叉验证、数据日期和来源状态。宏观数据只作验证，不强行覆盖价格信号。

相关性矩阵先按日期取两项资产的共同收盘观测，再计算简单日收益率和Pearson相关系数，避免不同市场节假日造成错位。方法页同时展示20日、60日和252日结果、实际样本数、计算区间、代理资产及模型限制。
